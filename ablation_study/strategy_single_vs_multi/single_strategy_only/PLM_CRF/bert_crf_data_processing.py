...
def generate_label_map():
    """生成BIOES标签映射"""
    entities = ['NR', 'NS', 'NB', 'NO', 'NG', 'T', 'ZD', 'ZZ', 'ZF', 'ZP', 'ZS', 'ZA']
    prefixes = ['B', 'I', 'E', 'S']
...
class NERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for word_id in word_ids:
            if word_id is None:
                aligned_labels.append(0)  # CRF要求填充为0
            else:
        ...
        aligned_labels += [0] * (self.max_len - len(aligned_labels))
...
def prepare_datasets(data_path, tokenizer, test_size=0.2):
    ...
    train_seq, val_seq, train_lbl, val_lbl = train_test_split(
        sequences, labels, test_size=test_size, random_state=42, shuffle=True
    )
    logger.info(f"数据集划分结果：\n"
                f"- 总序列数：{len(sequences)}\n"
                f"- 训练集序列数：{len(train_seq)}\n"
                f"- 验证集序列数：{len(val_seq)}")
    print("训练集标签分布:", Counter([l for seq in train_lbl for l in seq]))
    print("验证集标签分布:", Counter([l for seq in val_lbl for l in seq]))
    return (
