...
from bert_softmax_model import BERT_Softmax  # 导入 BERT+Softmax 模型
from bert_softmax_data_processing import generate_label_map  # 导入标签映射生成函数
...
def predict(model_path, tokenizer_path, test_path, output_path):
    ...
    # 加载模型和分词器
    try:
        model = BERT_Softmax.from_pretrained(model_path)
        model.to(device)
    ...
    with open(test_path, 'r', encoding='utf-8') as f_in, \
        ...
        for line_idx, line in enumerate(f_in):
            ...
            for chunk_idx in range(0, total_length, max_chunk_length):
                ...
                predictions = torch.argmax(logits, dim=-1).cpu().tolist()[0]  # 获取预测的标签 ID

                # 处理 word IDs 和标签对齐
                word_ids = inputs.word_ids(batch_index=0)
                valid_tags = []
                for word_id, tag_id in zip(word_ids, predictions):
                    if word_id is not None:
                        valid_tags.append((word_id, tag_id))  # 只保留有效的标签
...
if __name__ == "__main__":
    # 配置路径
    md_dir = "../models/bert_softmax_B_2025-2-22"  # 替换为实际模型路径
    model_path = md_dir
    tokenizer_path = md_dir
    test_file = "../../datasets/test/raw/testset_B.txt"  # 测试文件路径
    output_file = "../../results/result_B.txt"  # 输出文件路径
